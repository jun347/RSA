# IMPORTANT!!!!
# you have to install dash and pandas library 
...
def text_n(p, q):
    if p is not None and q is not None:
        if miller_rabin_one(int(p)) and miller_rabin_one(int(q)):
            return 'OK'
        else:
            return 'Not primes'
    else:
...
def text_e(e, reset, p, q):
    switch = ctx.triggered_id
    if switch == 'e':
        if miller_rabin_one(int(e)) == False and gcd(e, data_lambdan) == False:
            return 'Not a prime and the greatest common divisor with lambda(n) is not 1'
        elif miller_rabin_one(int(e)) == False:
            return 'Not a prime'
